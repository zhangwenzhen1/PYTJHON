...
rl = pd.read_csv('D:\guangdong\zhoumangshi0114-0120.csv',encoding='gbk')
...
# zb['流量系数'] = np.where(zb['载波带宽(MHZ)'] ==10,2*zb['流量系数'],
#                      np.where(zb['载波带宽(MHZ)'] ==15,20/15*zb['流量系数'],
#                               np.where(zb['载波带宽(MHZ)'] ==5,4*zb['流量系数'],zb['流量系数'])))
...
gg.columns = ['日流量为0','流量系数大于0小于0.05的小区数','流量系数大于0.05小于0.2的小区数',
              '流量系数0.2~0.5','流量系数0.5~1.5','流量系数1.5~3','流量系数3~5','流量系数>5']
...
zb['路测网格'] = np.where(zb['路测网格'].str.startswith('A'),'A',
                      np.where(zb['路测网格'].str.startswith('C'),'C',
                               np.where(zb['路测网格'].str.startswith('D'),'D','B')))
...
writer = pd.ExcelWriter('D:\guangdong\Result\zhoumangshi1224-1230.xlsx')
# zb.to_excel(writer,'流量系数总表',)
xishufenbu.to_excel(writer,'系数分布',index=False)
gg.to_excel(writer,'均衡比',)
wg.to_excel(writer,'网格均衡比',)
gfq.to_excel(writer,'高负荷清单',index=False)
gff.to_excel(writer,'高负荷统计',)
wq.to_excel(writer,'地市网格流量占比',)
writer.save()